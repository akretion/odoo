...
class BaseModel(MetaModel('DummyModel', (object,), {'_register': False})):
    ...
    *   :class:`AbstractModel` for abstract super classes meant to be shared by
    ...
    @api.model
    def _default_is_simulation(self):
        active_ids = self._context.get('active_ids', [])
        active_model = self._context.get('active_model')
        if active_model and active_ids and len(active_ids) == 1:
            origin = self.env(active_model).browse(active_ids[0])
            print("DEFAULT IS SIM", origin.is_simulation)
            if hasattr(origin, "is_simulation") and origin.is_simulation:
                return True
        return False

    def _inverse_is_simulation(self):
        for rec in self:
            if rec.is_simulation:
                print("is SIMUL", rec)
                rec.sudo().active = False  # raise UserError if no active field!
    ...
    def _add_magic_fields(self):
        ...
        * $CONCURRENCY_CHECK_FIELD is a computed field with its computing
        ...
        if self._log_access:
            add('is_simulation', fields.Boolean(default="_default_is_simulation", inverse="_inverse_is_simulation"))
            add('active', fields.Boolean(default=True))
...
%(fields_list)s""") % {
    ...
    def _create(self, data_list):
        ...
        if self._log_access:
            ...
            if self.env.context.get("is_simulation"):
                columns0.append(('is_simulation', "%s", True))
                # TODO fill simulation_parent_id or active_id
    ...
    def _where_calc(self, domain, active_test=True):
        ...
        if self._active_name and active_test and self._context.get('active_test', True):
            ...
            if not any(item[0] == self._active_name for item in domain):
                domain = [(self._active_name, 'in', (True, False))] + domain
#                print("domain -----", self, domain)
#                domain = [(self._active_name, '=', 1)] + domain
        elif self._active_name:
            domain = [('active', 'in', (True, False))] + domain
    ...
    def __iter__(self):
        ...
        if len(self._ids) > PREFETCH_MAX and self._prefetch_ids is self._ids:
            ...
            for id in self._ids:
                item = self._browse(self.env, (id,), self._prefetch_ids)
                # FIXME do it also if > PREFETCH_MAX
                if self.pool.ready and hasattr(item, "is_simulation") and item.is_simulation:
                    yield item.with_context(
                        is_simulation=True,
                        simulation_parent_res="%s,%s" % (self._table, item.id),
                    )
                else:
                    yield item
